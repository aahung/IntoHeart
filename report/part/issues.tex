...
\paragraph{Not well handle Bluetooth connection on and loss}
Currently connection on or loss cannot be handled user friendly, when connection is on, the data starts flowing, while not obvious UI changes made. Either is connection loss. 
\paragraph{Not well handle Sensor loosing}
The heart rate sent from sensor will become weird, so it needs to be filtered.